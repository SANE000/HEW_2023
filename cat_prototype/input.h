...
// プロトタイプ宣言
//*****************************************************************************
HRESULT InitInput(HINSTANCE hInst, HWND hWnd);//初期化一回呼び出す
void UninitInput(void);//終了処理 最後に一回
void UpdateInput(void);//1フレームに一回呼び出す

//---------------------------- keyboard
//全部のキーボードボタンの状態を取ってきてくれる！すごい
bool GetKeyboardPress(int nKey);//おしっぱOK
bool GetKeyboardTrigger(int nKey);//おしっぱでも一回しか取れない
bool GetKeyboardRepeat(int nKey);//最初はトリガー時間経過でプレス
bool GetKeyboardRelease(int nKey);//離した時に取ってきてくれる
...
//---------------------------- game pad//コントローラーは
BOOL IsButtonPressed(int padNo,DWORD button);//プレスと
BOOL IsButtonTriggered(int padNo,DWORD button);//トリガーだけ
